...
public class UserService {
	...
	public ResponseEntity<BaseResponse<Void>> signup(SingUpRequestDto dto) {
		...
		return BaseResponse.ok("회원가입을 성공하셨습니다", null, HttpStatus.CREATED); //201 반환
	}
	...
	public ResponseEntity<BaseResponse<Void>> login(LoginRequestDto dto, HttpServletResponse response) {
		...
		if (findAdmin.isEmpty()) {                                                              //이메일이 존재하지 않다 반환 시 찾을 때까지 이메일 무한 입력 가능성
			return BaseResponse.error("계정이 존재하지 않습니다.", null, HttpStatus.UNAUTHORIZED); //401 반화 (인증되지 않은 사용자가 접근 시도)
		}
		...
		return BaseResponse.ok("로그인 성공", null, HttpStatus.OK);

	}

	// Access Token 만료 시 Refresh Token으로 재발급하는 코드
	public ResponseEntity<BaseResponse<Void>> reissue(String refreshToken, HttpServletResponse response) {
		...
		if (!jwtTokenProvider.validate(refreshToken)) {
			return BaseResponse.error("Refresh Token이 유효하지 않습니다.", null, HttpStatus.UNAUTHORIZED); //401 반환
		}

		//요청을 한 사람이 기존에 회원가입이 되어 있는 관리자가 맞는지 검사
		Long userId = jwtTokenProvider.parseJwt(refreshToken).getUserId();
		if (userRepository.findById(userId).isEmpty()) {
			return BaseResponse.error("관리자 정보를 찾을 수 없습니다.", null, HttpStatus.UNAUTHORIZED); //401 반환
		}
		...
		if (tokenRepository.findByUserId(userId).isEmpty()) {
			return BaseResponse.error("서버에서 발급한 토큰이 아닙니다.", null, HttpStatus.UNAUTHORIZED); //401 반환
		}
		...
		response.setHeader("Authorization", "Bearer " + loginResponseDto.accessToken());
		response.setHeader("Refresh", loginResponseDto.refreshToken());
		return BaseResponse.ok("재발급 성공", null, HttpStatus.OK); //200 반환
	}

	public ResponseEntity<BaseResponse<Void>> logout(String accessToken, RefreshTokenRequestDto requestrefreshToken) {
		//토큰 구조 먼저 확인
		...
		if (!jwtTokenProvider.validate(accessToken)) {
			return BaseResponse.error("유효하지 않는 토큰", null, HttpStatus.UNAUTHORIZED); //401 반환
		}
		...
		if (findrefreshToken.isEmpty()) {
			return BaseResponse.error("토큰이 존재하지 않습니다", null, HttpStatus.UNAUTHORIZED); // 401 반환
		}
		RefreshToken refreshToken = findrefreshToken.get();

		if (!refreshToken.getRefreshToken().equals(requestrefreshToken.getRefreshToken())) {
			return BaseResponse.error("Refresh Token이 일치하지 않습니다.", null, HttpStatus.UNAUTHORIZED); // 401 반환
		}
		...
		return BaseResponse.ok("로그아웃 성공", null, HttpStatus.OK); //반환이 없으므로 204로 보내면 body값을 비워서 보내주므로 200
	}
