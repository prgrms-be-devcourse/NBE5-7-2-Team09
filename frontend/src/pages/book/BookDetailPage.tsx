...
const BookDetailPage = () => {
  ...
  const [isLibrarySelectDialogOpen, setIsLibrarySelectDialogOpen] =
    useState(false);
  const [newLibraryName, setNewLibraryName] =
    useState<string>("내 라이브러리1");
    ...
    try {
      ...
      if (createResponse.data) {
        ...
        if (createResponse.data.data) {
          console.log(
            "응답 데이터 내부 데이터 키:",
            Object.keys(createResponse.data.data)
          );
        }
      }
      ...
      if (
        ...
        createResponse.data &&
        createResponse.data.data &&
        createResponse.data.data.library_id
      ) {
      ...
      if (!newLibraryId) {
        ...
        const createdLibrary = librariesResponse.data.libraries?.find(
          (lib) => lib.library_name === newLibraryName
        );
      ...
      if (!newLibraryId) {
        throw new Error(
          "라이브러리 ID를 찾을 수 없습니다. 서버 응답 형식이 예상과 다릅니다."
        );
      }
  ...
  return (
    <div className="pb-12">
      ...
      {/* 라이브러리 선택 다이얼로그 */}
      <Dialog
        open={isLibrarySelectDialogOpen}
        onOpenChange={setIsLibrarySelectDialogOpen}
      >
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>책을 추가할 라이브러리 선택</DialogTitle>
            <DialogDescription>
              {book.name}을(를) 추가할 라이브러리를 선택해주세요.
            </DialogDescription>
          </DialogHeader>
          ...
          {isLoadingLibraries ? (
            ...
            <div className="py-4">
              <RadioGroup
                ...
                onValueChange={(value) => setSelectedLibraryId(parseInt(value))}
                className="space-y-2"
              >
                {libraries.map((library) => (
                  ...
                  >
                    <RadioGroupItem
                      value={library.library_id.toString()}
                      id={`library-${library.library_id}`}
                    />
                    <Label
                      htmlFor={`library-${library.library_id}`}
                      className="flex-1"
                    >
      ...
      >
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              새 라이브러리를 생성하고 {book.name}을(를) 추가합니다.
            </DialogDescription>
          </DialogHeader>
